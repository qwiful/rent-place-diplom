...
const { CONTRACT_STATUSES, validateEnum } = require('../utils/enums')
...
const createContract = async (req, res) => {
  try {
    ...
    const statusCheck = validateEnum(status, CONTRACT_STATUSES, 'status')
        ...
        (tenant_organization_id
          ? (
              await prisma.organizations.findUnique({
                where: { id: tenant_organization_id },
              })
            ).representative_id
          : null)
      if (tenantId) {
        await createNotification(
          tenantId,
          'Договор заключён',
          `Договор №${contractNumber} вступил в силу`,
          'Contract',
          contract.id,
        )
      }
...
const updateContract = async (req, res) => {
  try {
    ...
    console.log('ID помещения из договора:', existing.rental_object_id)
...
const getExpiringContracts = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Ошибка при получении истекающих договоров' })
  }
}
...
const getContractsByTenant = async (req, res) => {
  try {
    ...
    if (req.user.roles.name === 'tenant') {
      ...
      const isOwnIndividual = type === 'individual' && tenantIdInt === user.id
      const isOwnLegal =
        type === 'legal' && tenantIdInt === user.organization_id
      if (!isOwnIndividual && !isOwnLegal) {
        return res
          .status(403)
    ...
    res.status(500).json({ error: 'Ошибка при получении договоров арендатора' })
  }
}
  ...
  try {
    ...
    if (status === 'active') {
      const activeContract = await prisma.contracts.findFirst({
        where: {
          rental_object_id: existing.rental_object_id,
          status: 'active',
          id: { not: contractId },
        },
      })
      if (activeContract) {
        return res
          .status(400)
          .json({ error: 'На это помещение уже есть активный договор' })
      }
    }
    ...
    if (status === 'active') {
      await prisma.rental_objects.update({
        where: { id: existing.rental_object_id },
    ...
    if (status === 'active') {
      const tenantUserId =
        existing.tenant_user_id ||
        (existing.tenant_organization_id
          ? (
              await prisma.organizations.findUnique({
                where: { id: existing.tenant_organization_id },
              })
            ).representative_id
          : null)
      if (tenantUserId) {
        await createNotification(
          ...
          'contract',
          'Договор активирован',
          `Договор №${existing.contract_number} вступил в силу.`,
          'Contract',
          contractId,
        )
      }
    ...
    res.json({ message: 'Статус договора обновлен', contract: updated })
  } catch (error) {
