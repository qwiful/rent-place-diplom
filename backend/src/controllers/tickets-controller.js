...
const createTicket = async (req, res) => {
  try {
    ...
    const typeCheck = validateEnum(type, SERVICE_TYPES, 'type')
    if (!typeCheck.valid) {
      return res.status(400).json({ error: typeCheck.error })
    }

    const priorityCheck = validateEnum(priority, PRIORITIES, 'priority')
    if (!priorityCheck.valid) {
      return res.status(400).json({ error: priorityCheck.error })
    }
...
const updateTicketStatus = async (req, res) => {
  try {
    ...
    const statusCheck = validateEnum(status, TICKET_STATUSES, 'status')
