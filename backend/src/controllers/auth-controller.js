...
const register = async (req, res) => {
  try {
    const { email, password, phone, firstName, lastName, middleName, role } =
      req.body
    ...
    if (role === 'landlord') {
      ...
      if (!managerRole) {
        return res.status(500).json({ error: 'Роль manager не найдена в БД' })
      }
      ...
      if (!tenantRole) {
        return res.status(500).json({ error: 'Роль tenant не найдена в БД' })
      }
    ...
    const token = jwt.sign(
      {
        userId: result.id,
        email: result.email,
        role: role === 'landlord' ? 'manager' : 'tenant',
      },
